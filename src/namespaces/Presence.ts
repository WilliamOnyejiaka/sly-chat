...
export default class Presence extends Base {
    ...
    //         const successful = await Presence.onlineCustomer.set(userId, {
    //             socketId: socket.id
    //         });
    //         if (!successful) {
    ...
    //         const successful = await Presence.onlineVendor.set(userId, {
    //             socketId: socket.id
    //         });
    //         if (!successful) {
    //             socket.emit('appError', {
    ...
    //     const repoResult = userType == UserType.Customer ? await Presence.chatRepo.getBuyerChatsWithMessages(userId) : await Presence.chatRepo.getSellerChatsWithMessages(userId);
    ...
    //     const updateOfflineMessagesRepoResult = await Presence.messageRepo.updateOfflineMessages(chatIds, userId, userType);
